...
public class BookingRepository
{
    ...
    public async Task<Booking> CreateBookingAsync(int userId, int flightId)
    ...
    public async Task<List<Booking>> GetBookings(int? userId, int? flightId)
    {
       ...
       .AsQueryable();
        ...
        if (userId.HasValue)
        {
            query = query.Where(b => b.PassengerId == userId.Value);
        }
